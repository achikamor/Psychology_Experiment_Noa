...
def update_position2(canvas, id, planet_obj1, path_iter, cont):
    pressed = False
    try:
    ...
    # repeat after delay
    canvas.after(DELAY, update_position2, canvas, id, planet_obj1, path_iter, cont)
    if pressed:
...
def main(client, iter, is_prac):
    ...
    for i in range(iter):
        ...
        if circular:
            ...
            top.after(DELAY, update_position2, canvas, planet1, planet_obj1, path_iter, cont)
            circular = False
        else:
            start_t = time.time()
    ...
    if is_prac:
        client.blocks['beep_practice'] = result
    else:
        client.blocks['beep_task'] = result
